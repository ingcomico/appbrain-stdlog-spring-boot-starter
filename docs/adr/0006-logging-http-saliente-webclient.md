...
> **Nota de rama (`spring-boot-3.x`):** la decisión y el código aplican igual. En esta rama `StdlogClientHttpPayload` usa `HttpHeaders.entrySet()` (no `headerSet()`) y las dependencias `spring-webflux` / `reactor-core` las gobierna el BOM de Spring Boot 3.5.x. El `BeanPostProcessor` (en vez de `WebClientCustomizer`) hace que `StdlogWebClientAutoConfiguration` sea idéntica en las dos ramas.
