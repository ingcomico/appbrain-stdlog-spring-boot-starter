...
- Los ADR en `docs/adr/` son los mismos que en `main`. `ADR-0001`, `0002`, `0003` y `0006` llevan una "Nota de rama" al inicio con lo que difiere en esta línea. `ADR-0007` (R2DBC) y `ADR-0008` (soporte WebFlux; Fases 1 y 2-parcial hechas, resto pendiente) aplican sin diferencias.
...
| Entrada WebFlux (`ADR-0008` Fases 1-2) | `spring-webflux` `provided`; `StdlogWebFilter` + lectura del Reactor Context en `StdlogWebClientExchangeFilter` | **idéntico** (usa API de Spring 6/7 común: `WebFilter`, `HandlerMapping`, decoradores reactivos, `Mono.deferContextual`) |
...
- Suite: 218 tests, 0 fallos, verificado en JDK 17 y JDK 25.
