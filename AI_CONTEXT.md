...
3. Revisar los ADR relevantes dentro de `docs/adr/`. ADRs vigentes (todos en estado `Aceptado`):
   ...
   - `0014` — backend de logging: Logback y Log4j2 con salida equivalente. Atiende el fondo de F-13. **Implementado.**
...
- `appbrain.stdlog.core`: primitives transversales:
  ...
  - `backend.StdlogEventWriter` con `LogbackEventWriter`, `Log4j2EventWriter` y `FallbackEventWriter`, elegidos por `backend.StdlogBackend` (`ADR-0014`);
...
- **Politica de dependencias** (revisada al cerrar la auditoria F-13): lo que soporta una capacidad *opcional* va en `provided`, para que solo lo cargue quien la use — `spring-webmvc`, `spring-webflux`, `reactor-core`, `r2dbc-proxy`, `r2dbc-spi`, `context-propagation`, `log4j-api`, `jakarta.servlet-api`. Lo que sostiene una capacidad *central* va en `compile`, aunque eso lo lleve a consumidores que no la usen: `datasource-proxy` (logging JDBC) y `logstash-logback-encoder` (salida JSON por defecto).
  Se evaluo pasar `datasource-proxy` a `provided` por simetria con `r2dbc-proxy` y **se descarto**: su version no la gestiona el BOM de Spring Boot, asi que cada consumidor de logging JDBC tendria que declarar dependencia *y* version, con el riesgo de deriva que eso trae, a cambio de ahorrar 132 KB a quien no use JDBC. Aqui la implementacion era correcta y el principio estaba enunciado de forma demasiado absoluta.
- OpenTelemetry se consulta por reflexion, sin dependencia (`ADR-0002`).
...
- Logback/logstash encoder (v9.0) es el mecanismo de salida JSON provisto. Ya **no es el unico**: desde `ADR-0014` el starter detecta el backend enlazado (por `LoggerFactory.getILoggerFactory()`) y emite por el escritor correspondiente, con salida equivalente. Logback usa el `Marker` de logstash; Log4j2 usa `ObjectMessage` con la API de Log4j2 directamente, porque eso no se alcanza desde SLF4J. Con un backend no soportado se usa un respaldo de texto que **no pierde el evento**, y se avisa con `WARN` al arrancar. Un test compara el JSON de los dos caminos, y se verifico que detecta divergencias.
...
- El body de una respuesta saliente se omite del log cuando su `Content-Length` supera `stdlog.restclient.max-capture-bytes` (256 KiB por defecto): para loguearlo habria que bufferizarlo entero, y eso es lo que se acota. La respuesta que recibe la aplicacion nunca se recorta.
