...
- Los ADR en `docs/adr/` son los mismos que en `main`. `ADR-0001`, `0002`, `0003` y `0006` llevan una "Nota de rama" al inicio con lo que difiere en esta línea. `ADR-0007` (R2DBC) y `ADR-0008` (soporte WebFlux; Fase 1 hecha, Fases 2-3 pendientes) aplican sin diferencias.
...
| Entrada WebFlux (`ADR-0008` Fase 1) | `spring-webflux` `provided`; `StdlogWebFilter` | **idéntico** (usa API de Spring 6/7 común: `WebFilter`, `HandlerMapping`, decoradores reactivos) |
...
- Suite: 216 tests, 0 fallos, verificado en JDK 17 y JDK 25.
